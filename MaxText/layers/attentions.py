...
class AttentionOp(nn.Module):
  ...
  num_query_heads: int
  num_kv_heads: int
  prefill_key_axis_order: list[int]
  prefill_value_axis_order: list[int]
  ar_key_axis_order: list[int]
  ar_value_axis_order: list[int]
  ...
  def revert_kvlen_axis(self, kv, axis_order):
    ...
    """
    return jax.numpy.moveaxis(kv, (0, 1, 2, 3), axis_order)

  def move_kvlen_axis(self, kv, axis_order):
    ...
    """
    axis_order_to_index_mapping = {a:i for i, a in enumerate(axis_order)}
    axis_to_move_to = [i for a, i in sorted(axis_order_to_index_mapping.items())]
    return jax.numpy.moveaxis(kv, (0, 1, 2, 3), axis_to_move_to)

  def cached_kv_layout(self, kv_layout, cached_axis_order):
    return [kv_layout[i] for i in cached_axis_order]
  ...
  def cached_kv_shape(self, kv_shape, cached_axis_order):
    ...
    """
    return [kv_shape[i] for i in cached_axis_order]
  ...
  def _get_prefill_cache(self, batch, heads, kv_head_size, quantize_kvcache):
    ...
    prefill_cache_logical_layout = ("cache_batch", "cache_sequence", "cache_heads", "cache_kv")
    prefill_key_layout = self.cached_kv_layout(prefill_cache_logical_layout, self.prefill_key_axis_order)
    prefill_value_layout = self.cached_kv_layout(prefill_cache_logical_layout, self.prefill_value_axis_order)

    prefill_cache_logical_shape = (batch, self.max_prefill_predict_length, heads, kv_head_size)
    prefill_key_shape = self.cached_kv_shape(prefill_cache_logical_shape, self.prefill_key_axis_order)
    prefill_value_shape = self.cached_kv_shape(prefill_cache_logical_shape, self.prefill_value_axis_order)

    cached_key = self.variable(
        "cache",
        "cached_prefill_key",
        nn.with_logical_partitioning(jnp.zeros, prefill_key_layout),
        prefill_key_shape,
        dtype,
    )
    cached_value = self.variable(
        ...
        nn.with_logical_partitioning(jnp.zeros, prefill_value_layout),
        prefill_value_shape,
        dtype,
    )
    cached_segment_id = self.variable(
        ...
        (prefill_cache_logical_shape[0], self.max_prefill_predict_length),
        jnp.int32,
    )

    if self.quantize_kvcache:
      prefill_cache_logical_shape_scale = (batch, self.max_prefill_predict_length, heads, 1)
      prefill_key_shape_scale = self.cached_kv_shape(prefill_cache_logical_shape_scale, self.prefill_key_axis_order)
      prefill_value_shape_scale = self.cached_kv_shape(prefill_cache_logical_shape_scale, self.prefill_value_axis_order)
      cached_key_scale_var = self.variable(
          "cache",
          "cached_prefill_key_scale",
          nn.with_logical_partitioning(jnp.zeros, prefill_key_layout),
          prefill_key_shape_scale,
          jnp.bfloat16,
      )
  ...
  def _get_ar_cache(self, batch, heads, kv_head_size, quantize_kvcache):
    ...
    ar_cache_logical_layout = ("cache_batch", "cache_sequence", "cache_heads", "cache_kv")
    ar_key_layout = self.cached_kv_layout(ar_cache_logical_layout, self.ar_key_axis_order)
    ar_value_layout = self.cached_kv_layout(ar_cache_logical_layout, self.ar_value_axis_order)

    ar_cache_logical_shape = (batch, cache_length, heads, kv_head_size)
    ar_key_shape = self.cached_kv_shape(ar_cache_logical_shape, self.ar_key_axis_order)
    ar_value_shape = self.cached_kv_shape(ar_cache_logical_shape, self.ar_value_axis_order)

    cached_key = self.variable(
        "cache",
        "cached_ar_key",
        nn.with_logical_partitioning(jnp.zeros, ar_key_layout),
        ar_key_shape,
        dtype,
    )
    cached_key.value = nn.with_logical_constraint(
        cached_key.value, ar_key_layout
    )

    cached_value = self.variable(
        "cache",
        "cached_ar_value",
        nn.with_logical_partitioning(jnp.zeros, ar_value_layout),
        ar_value_shape,
        dtype,
    )
    cached_value.value = nn.with_logical_constraint(
        cached_value.value, ar_value_layout
    )
    ...
    cached_segment_id = self.variable(
        ...
        "cache_ar_segment_id",
        nn.with_logical_partitioning(jnp.zeros, ("cache_batch", "cache_sequence")),
        (ar_cache_logical_shape[0], cache_length),
        jnp.int32,
    )

    if self.quantize_kvcache:
      ar_cache_logical_shape_scale = (batch, cache_length, heads, 1)
      ar_key_shape_scale = self.cached_kv_shape(ar_cache_logical_shape_scale, self.ar_key_axis_order)
      ar_value_shape_scale = self.cached_kv_shape(ar_cache_logical_shape_scale, self.ar_value_axis_order)
  ...
  def update_ar_key_value(
    ...
    one_token_key = self.move_kvlen_axis(one_token_key, self.ar_key_axis_order)
    one_token_value = self.move_kvlen_axis(one_token_value, self.ar_value_axis_order)
...
class Attention(nn.Module):
  ...
  def __call__(
    ...
    attention_op = AttentionOp(
        ...
        dtype=self.dtype,
        prefill_key_axis_order=[int(i) for i in self.config.prefill_key_axis_order.split(",")],
        prefill_value_axis_order=[int(i) for i in self.config.prefill_value_axis_order.split(",")],
        ar_key_axis_order=[int(i) for i in self.config.ar_key_axis_order.split(",")],
        ar_value_axis_order=[int(i) for i in self.config.ar_value_axis_order.split(",")],
    )
